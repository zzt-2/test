      ...
      <q-card-section>
        ...
        <div class="q-gutter-sm q-mt-sm">
          ...
          <div class="q-gutter-sm">
            ...
            <q-btn :loading="buttonLoading" color="primary" label="加载按钮" @click="toggleButtonLoading" />
          </div>
        </div>
      </q-card-section>
    </q-card>
      ...
      <q-card-section>
        ...
        <div class="row q-col-gutter-md q-mt-sm">
          <div class="col-12 col-md-6">
            <q-input v-model="textInput" label="文本输入框" outlined clearable hint="这是一个文本输入框"
        ...
        <div class="row q-col-gutter-md">
          <div class="col-12 col-md-6">
            <q-select v-model="selectValue" :options="selectOptions" label="选择框" outlined emit-value map-options />
          </div>
          <div class="col-12 col-md-6">
            <q-select v-model="multiSelectValue" :options="selectOptions" label="多选框" outlined multiple emit-value
              map-options />
          </div>
          <div class="col-12 col-md-6">
            <q-select v-model="filteredSelectValue" :options="selectOptions" label="可搜索选择框" outlined use-input
              @filter="onSelectFilter" emit-value map-options />
          </div>
          <div class="col-12 col-md-6">
            <q-select v-model="groupSelectValue" :options="groupedSelectOptions" label="分组选择框" outlined emit-value
              map-options />
          </div>
          <div class="col-12">
            <q-select v-model="themeSelectValue" :options="themeOptions" label="主题设置" outlined emit-value map-options
        ...
        <div class="q-gutter-md q-mt-sm">
          <div>
            ...
            <div class="q-gutter-sm">
              ...
              <q-checkbox v-model="checkboxIndeterminate" indeterminate-value="maybe" label="不确定状态复选框" />
              <q-checkbox v-model="checkboxDisabled" disable label="禁用复选框" />
              <div class="q-gutter-sm">
                <q-checkbox v-for="item in checkboxList" :key="item" v-model="selectedCheckboxes" :val="item"
                  :label="item" />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="q-gutter-sm">
              ...
              <q-toggle v-model="toggleValue4" checked-icon="mdi-check" unchecked-icon="mdi-close" label="自定义图标开关" />
            </div>
          </div>

          <div>
            <div class="text-subtitle2 q-mb-sm">选项按钮 (QOptionGroup)</div>
            <q-option-group v-model="optionGroupValue" :options="optionGroupOptions" color="primary" />
          </div>
        </div>
      </q-card-section>
    </q-card>
      ...
      <q-card-section>
        ...
        <div class="q-gutter-md q-mt-sm">
          ...
          <div>
            ...
            <div class="text-caption q-mt-sm">最小值: {{ rangeValue.min }}, 最大值: {{ rangeValue.max }}</div>
          </div>

          <div>
            <div class="text-subtitle2 q-mb-sm">刻度滑块</div>
            <q-slider v-model="sliderValueWithMarkers" :min="0" :max="10" :step="1" markers label color="accent" />
          </div>
          ...
          <div>
            ...
            <div class="row">
              <div style="width: 60px; height: 200px">
                <q-slider v-model="verticalSliderValue" :min="0" :max="100" vertical label color="primary" />
              </div>
            </div>
          </div>
        </div>
      ...
      <q-card-section>
        ...
        <div class="q-gutter-md q-mt-sm">
          ...
          <div>
            ...
            <div class="q-gutter-sm">
              <q-chip color="primary" text-color="white" icon="mdi-bookmark" label="主要标签" clickable
                @click="notify('点击标签')" />
              <q-chip color="secondary" text-color="white" removable @remove="notify('删除标签')" label="可删除标签" />
              <q-chip color="accent" text-color="white" clickable avatar="https://cdn.quasar.dev/img/avatar.png"
                label="头像标签" />
              <q-chip v-for="item in chipList" :key="item" color="info" text-color="white" removable
                @remove="removeChip(item)" :label="item" />
            </div>
          </div>
      ...
      <q-card-section>
        ...
        <div class="q-gutter-md q-mt-sm">
          <div>
            ...
            <q-linear-progress :value="progressValue" size="20px" color="positive" class="q-mt-md" />
          </div>
          ...
          <div>
            ...
            <div class="row q-gutter-md">
              ...
              <q-circular-progress :value="progressValue" size="80px" color="secondary" show-value />
      ...
      <q-card-section>
        ...
        <div class="q-gutter-md q-mt-sm">
          <div>
            ...
            <q-table class="my-border-default" :rows="tableRows" :columns="tableColumns" row-key="name"
              :alternating-rows="true" :pagination="{ rowsPerPage: 5 }" flat bordered />
          </div>
          ...
          <div>
            ...
            <q-table :rows="tableRows" :columns="tableColumns" row-key="name" :selected="selectedTableRows"
              :selection="tableSelection" @update:selected="updateTableSelection" flat bordered />
          </div>
        </div>
      </q-card-section>
    </q-card>
...
const themeOptions = computed(() => themes.map(theme => ({ label: theme.label, value: theme.id })));
